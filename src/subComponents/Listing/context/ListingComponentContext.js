...
export function useListingComponentContext() {
  return useContext(ListingComponentContext);
}

export function ListingComponentContextProvider(props) {
  const customUseEffect = useEffect;
  ...
  if (!!filtersQuery) {
    ...
    customUseEffect(() => {
      ...
      const isMoreThanOneCategory = targetData?.some((item) => item?.values?.length > 1);
